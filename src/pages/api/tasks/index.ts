...
async function get(req: NextApiRequest, res: NextApiResponse) {
  const completedOffset = Number(req.query.completedOffset ?? "0");
  const completedLimit = Number(req.query.completedLimit ?? DEFAULT_COMPLETED_PAGE_SIZE);
  ...
  const results = await Promise.all(
    accounts.map(async (account) => {
      try {
        ...
        const listRes = await tasks.tasks.list({
          tasklist: taskListId,
          showCompleted: true,
          showHidden: true,
...
async function post(req: NextApiRequest, res: NextApiResponse) {
  ...
  const taskListId = taskLists.data.items?.[0]?.id;
  if (!taskListId) {
    return res.status(400).json({ error: "Brak listy zadań na koncie" });
  }
